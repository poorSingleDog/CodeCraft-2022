...
public class Combine12 {        //所有时刻每个边缘固定上限--295w
...
//        });
        ...
        for(int i=0;i<Client.times;i++){                //----------------------------------------------------赋上限
            int[] temp_upper=new int[servers.length];       //存每个边缘在这一时刻的暂时上限
            int sum_c_remain=0;                 //该时刻客户总剩余需求
...
//                    aliveS.get(j).upper_limit=aliveS.get(j).band_width;
                    temp_upper[aliveS.get(j).num]=aliveS.get(j).band_width;
                    sum_c_remain-=aliveS.get(j).band_width;
                    upper=(int)((double)sum_c_remain/(aliveS.size()-j-1))+1;
                }else {                                     //容量》上限
//                    aliveS.get(j).upper_limit=upper;
                    temp_upper[aliveS.get(j).num]=upper;
                }
            }
            for(int j=0;j<servers.length;j++){
                servers[j].upper_limit=Math.max(servers[j].upper_limit,temp_upper[j]);
            }
        }
        for(Server s:servers){
            s.upper_limit=(int)((double)s.upper_limit*0.965);
        }
        long t2=System.currentTimeMillis();

        for(int i=0;i<Client.times;i++){        //----------------------------------------------------------开始分配
            for(Server s:servers)
                s.rest=s.upper_limit;
            //计算活的边缘节点（全部边缘节点-用于5%的边缘节点-自闭边缘节点）
            List<Server> aliveS=new ArrayList<>();
            for(int j=0;j<servers.length;j++){
                if(servers[j].sacri_times.contains(i)||servers[j].isDead){
            ...
            for(Server s:aliveS){
                ...
                for(Client c:s.clientList){
                    ...
                    if(s.rest>c.time_remain.get(i)){     //客户满
                    ...
                    }else if(s.rest<=c.time_remain.get(i)){      //边缘满
                        int trans=s.rest;
...
//        }
        System.out.println("赋上限："+(t2-t1));
        System.out.println("分配和检查客户："+(t3-t2));
    }
}
